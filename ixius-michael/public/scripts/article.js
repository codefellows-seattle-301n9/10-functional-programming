...
(function (module) {
  ...
  Article.numWordsByAuthor = () => {
    ...
    return Article.allAuthors().map(author => {
      return {
        name: author,
        numWords: Article.all.filter(obj => author === obj.author).map(article => {
          return article.body.split(' ').length;
        }).reduce((acc, cur) => acc + cur)
  ...
  //due to the iify Article is nolonger in the global scope, attaching it to module makes it avalible again//
